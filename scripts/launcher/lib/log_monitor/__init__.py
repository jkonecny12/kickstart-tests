...
class LogMonitor(object):
    def __init__(self, install_log, timeout, log_request_handler_class=None):
        """Monitor VM logs and reacts properly on what happen there.

        :param str install_log: path where to store installation log
        :param int timeout: set timeout in minutes before killing the VM
        :param log_request_handler_class: class compatible with pylorax.monitor.LogRequestHandler
        ...
        if not is_dry_run():
            self._lorax_log_monitor = LoraxLogMonitor(self._install_log,
                                                      self._timeout,
                                                      self._log_request_handler_class)
